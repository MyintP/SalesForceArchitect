...
for notes (bottom). Twenty-four sheets with real teaching content — not just reading lists —
...
Management, Development Lifecycle), the agentic-enterprise module, a 47-question scored
Quiz Bank, hands-on Connected Case scenario practice, an Artefact Studio, a Pre-Sales / Solution
Engineer track (discovery, MEDDPICC, demo and POC craft, interview prep), and the
...
2. Use the left-hand map to move through Start → The Role → Foundations → Architecture
   Domains → Practice → Pre-Sales Track → Certification → Interview Bridge → Reference → Progress
...
4. In **Quiz Bank**, answer is scored immediately with an inline explanation; filter by
   domain or run all 47 questions, and progress is saved automatically
